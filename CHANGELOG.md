...
### Добавлено
 - Утилиты для логирования (getErrorMsg, getErrorToken) в адаптерах стали публичными

## [3.1.2] - 2026-09-17

### Добавлено

- **CLI `create from-flow` (кнопки с переходом)**: кнопка с `targetNodeId` теперь ведёт на блок-цель. Генерируется «именная» кнопка с payload `{ command: '[go:N]' }` и `bot.addAction('[go:N]', …)`: нажатие выполняет блок (ответ, действие, условие) либо команду/шаг, которым текст кнопки передаётся как ввод. Действия регистрируются до команд (слоты сравниваются вхождением), а ожидающий шаг пропускает такие нажатия (`return false`). Блоки, на которые ведут только кнопки, тоже генерируются. Без кнопок с переходом сгенерированный код не меняется.
- **CLI `create from-flow` (README и `.env`)**: генерируется `README.md` проекта — установка, токены, запуск, публичный HTTPS-адрес и подключение выбранных платформ; после генерации CLI печатает следующие шаги. Файл `.env` создаётся всегда (с пустыми переменными токенов выбранных платформ), при повторной генерации пустые переменные заполняются токенами из flow.json, заполненные не трогаются.
- **CLI `create from-flow` (приветствие и справка из настроек)**: без ноды welcome/help текст приветствия (с кнопками) и текст справки из настроек бота генерируются командами — раньше они не показывались никогда, так как стандартные интенты отключены (`intents: []`).
- **Telegram (`options.inline`)**: кнопка без payload/url с опцией `inline: true` показывается inline-кнопкой под сообщением; нажатие приходит боту как текст кнопки. Текст длиннее лимита `callback_data` (64 байта) передаётся токеном `#t<n>` и восстанавливается адаптером из клавиатуры сообщения. CLI выставляет опцию всем кнопкам — в сгенерированных ботах все кнопки Telegram inline.

### Изменено

- **Telegram (смешанные кнопки)**: если в сообщении есть inline-кнопки (url/payload), обычные текстовые кнопки тоже показываются inline, а не пропадают. Предупреждение о смешении клавиатур осталось только для кнопок запроса контакта/локации — inline они невозможны.

### Исправлено

- **CLI `create from-flow` (старт диалога)**: `/start` в Telegram и новая сессия Алисы/Маруси (запуск без команды) уходили в fallback — «не понял» вместо приветствия. Теперь приветствие срабатывает на `/start` (плюс стандартные или собственные слоты), а fallback вызывает его при `messageId === 0`. Шаг, ожидавший ответ в прошлой сессии, пропускает начало диалога (`return false`) и не съедает запуск.
- **CLI `create from-flow` (режим бота)**: поле `mode` игнорировалось — сгенерированный бот всегда работал в dev (подробные логи и отладочная информация доступны по URL вебхука). Теперь генерируется `bot.setAppMode(mode)`.
- **CLI `create from-flow` (`.env`)**: `env: './.env'` подключался, только если токены были во flow.json; токен, вписанный в `.env` вручную, бот не видел. Теперь `.env` подключается всегда. Без приветствия во flow.json CLI больше не подставляет своё «Привет!».
- **CLI `create from-flow` (незаданные переменные)**: `{{переменная}}`, которой ещё нет, выводилась в тексте как «undefined», а счётчик `cnt + 1` без начального значения давал `NaN`. Теперь в тексте подставляется пустая строка, в арифметике незаданная переменная считается нулём.
- **CLI `create from-flow` (условия isEmpty/isNotEmpty/contains)**: `isEmpty` генерировался как `!x`, и значение `0` считалось пустым; `contains` по незаданной переменной искал в строке «undefined». Теперь значение сравнивается как строка: пусто только отсутствующее значение или `''`.
- **CLI `create from-flow` (saveTo команды)**: ввод сохранялся после вывода текста (в ответе команды `{{переменная}}` была пустой) и в нижнем регистре. Теперь сохраняется до действий и текста, в исходном регистре (`originalUserCommand`).
- **CLI `create from-flow` (карточки)**: не генерировались заголовок и тип карточки (одна/список/галерея), `{{переменные}}` в картинке, кнопка картинки с переходом, а также карточки у шага и у блока «Действие». Теперь генерируются.
- **CLI `create from-flow` (эмоция, TTS, кнопки)**: эмоция ответа не генерировалась; `{{переменные}}` в TTS, заголовках кнопок и ссылках выводились как есть; «случайный порядок кнопок» у блока «Ответ» игнорировался. Исправлено.
- **CLI `create from-flow` (package.json)**: сгенерированный проект зависел от `umbot 3.1.0` независимо от версии CLI и не получал исправлений фреймворка, на которые опирается сгенерированный код. Теперь версия берётся из пакета, которым сгенерирован проект.
- **VK (`addAction`)**: кнопка клавиатуры с payload `{"command": "buy"}` приходит обычным сообщением, и `addAction('buy')` не срабатывал — в `userCommand` оставалась надпись кнопки. Теперь имя действия берётся из payload, как у callback-кнопок. Системная кнопка «Начать» (`{"command":"start"}`) по-прежнему приходит текстом.

- **CLI `create from-flow` (шаги)**: обработчик шага выводил свой текст до сохранения ответа пользователя и до выполнения действий шага — `{{переменная}}` в тексте подставлялась старым значением («Привет, undefined!»). Теперь шаг сохраняет ответ, выполняет действия и только потом выводит текст — так же, как показывает превью визуального редактора.
- **CLI `create from-flow` (блок «Ответ»)**: `isEnd` у standalone-блока response не генерировался — диалог не завершался, хотя завершение было задано на блоке. Теперь блок выставляет `ctrl.isEnd = true`, как и команда.
- **CLI `create from-flow` (условия eq/neq)**: сравнение генерировалось строгим `===`, поэтому ввод пользователя, сохранённый шагом строкой (`'42'`), никогда не совпадал с числом (`42` из JSON или `rand`). Теперь условия используют `isEqual` из сгенерированного `utils.ts`, который сравнивает значения как строки. Импорт добавляется только при наличии таких условий.
